<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == "POST")
  {
    ...
    if(!empty($commuter_username) && !empty($commuter_password) && !is_numeric($commuter_username))
    {
      //read to database
      $query = "select * from commuter where Username = '$commuter_username' limit 1";
        ...
        if($result && mysqli_num_rows($result) > 0)
            {
                ...
                if($user_data['Password'] === $commuter_password)
                {
                  $_SESSION['CommuterID'] = $user_data['CommuterID'];
                  header("Location: indexCommuter.php");
                  die;
                }
            }
      }
